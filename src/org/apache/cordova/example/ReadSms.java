...
public class ReadSms extends CordovaPlugin {
   ...
   //     Log.d(TAG, "Get texts from specified number.");
    ...
    private JSONArray readTextsFrom(String numberToCheck, Integer numberOfTexts
       ...
       // String[] smsNo = new String[] { "5554" }; //FOR demo and test purpose only
        String sortOrder = "date ASC"
                + ((numberOfTexts == READ_ALL) ? "" : " limit " + numberOfTexts);

        Cursor cursor = null;
        if("YES".equals(numberToCheck)){
        	cursor = contentResolver.query(Uri.parse("content://sms/inbox"), null,
     	           null, null, sortOrder);
        }else{
            String[] smsNo = new String[] { "89000045" }; //FOR demo and test purpose only
            cursor = contentResolver.query(Uri.parse("content://sms/inbox"), null,
        ...
        while (cursor.moveToNext()) {
            ...
            try {
               // current.put("time_received", cursor.getString(cursor.getString(cursor.getColumnIndex("date")));
                ...
                current.put("_id", cursor.getString(cursor.getColumnIndex("_id")));
                current.put("read", cursor.getString(cursor.getColumnIndex("read")));
